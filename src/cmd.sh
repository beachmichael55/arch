...
function prompt_choice() {
	...
	while true; do
		echo -e "${YELLOW}${prompt}${RESET}: (1) ${opt1}, (2) ${opt2}"
...
function prompt_per_package_choice() {
    ...
    while true; do
        echo -e "${YELLOW}How would you like to install '${app_name}'?${RESET}"
        echo -e "  (1) Native:  ${native_pkg}"
        echo -e "  (2) Flatpak: ${flatpak_pkg}"
        read -rp "Choice: " choice
        case "$choice" in
...
function install_one() {
    ...
    if pacman -Si "${package}" &> /dev/null; then
        # Official repo
        exec_log "sudo pacman -S --noconfirm --needed ${package}" "${GREEN}[pacman]${RESET} ${package}${warning_msg}"
    else
        # AUR
        if [[ -z "${AUR}" ]]; then
            log_msg "${RED}AUR helper not set. Cannot install ${package}.${RESET}"
            return 1
        fi
        exec_log "${AUR} -S --noconfirm --needed ${package}" "${GREEN}[AUR]${RESET} ${package}${warning_msg}"
    fi
}
...
function uninstall_one() {
    ...
    if pacman -Q ${package} &> /dev/null; then
        ...
        log_msg "${GREEN}[U]${RESET} Package ${package} is not installed."
    fi
}
