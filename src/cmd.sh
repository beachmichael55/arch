...
function prompt_per_package_choice() {
    ...
    while true; do
        ...
        echo -e "  (1) Native:  ${BLUE}${native_pkg}${RESET}"
        echo -e "  (2) Flatpak: ${BLUE}${flatpak_pkg}${RESET}"
        read -rp "Choice: " choice
        case "$choice" in
...
function install_one() {
    ...
    if pacman -Qi "${package}" &> /dev/null; then
        log_msg "${YELLOW}[I]${RESET} Package ${BLUE}${package}${RESET} is already installed."
        return
    fi
    local warning_msg=""
    ...
    for entry in "${lst_split[@]}"; do
        ...
        if [[ "$INSTALL_METHOD" == "Flatpak" && -n "$flatpak" ]]; then
			if flatpak info "$flatpak" &>/dev/null; then
				log_msg "${YELLOW}[I]${RESET} Flatpak package ${BLUE}${flatpak}${RESET} is already installed."
			else
