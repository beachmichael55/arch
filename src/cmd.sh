...
function exec() {
    ...
    if [[ "${VERBOSE}" == true ]]; then
        # Show output live and log
        eval "${command}" 2>&1 | tee -a "${LOG_FILE}"
        exit_status "${command}" "${PIPESTATUS[0]}"
    else
        # Check if command needs terminal input (basic check)
        if [[ -t 0 ]]; then
            # Run in foreground with logging, allows interaction
            eval "${command}" 2>&1 | tee -a "${LOG_FILE}"
            exit_status "${command}" "${PIPESTATUS[0]}"
        else
...
function exit_status() {
    ...
    echo "[INFO]: Exit status: ${status_code}" >>"${LOG_FILE}"
    if [[ ${status_code} -ne 0 ]]; then
        printf "%b\n" "\033[1A\033[2K${comment} ${RED}\u2718${RESET}"
        log_msg "${RED}Error: command failed: ${comment}${RESET}"
    else
        printf "%b\n" "\033[1A\033[2K${comment} ${GREEN}\u2714${RESET}"
    fi
}
