...
function configure_sysctl_tweaks() {
    ...
    if [ -f "$sysctl_file" ]; then
        exec_log "sudo rm $sysctl_file" "Removing existing sysctl performance tweaks"
    fi
...
EOF
" "Applying sysctl memory and kernel performance tweaks"

    # Reload sysctl settings
    exec_log "sudo sysctl --system" "Reloading sysctl parameters"
}
