...
function vm_drivers() {
    ...
    if [[ ${vm} =~ (^|[[:space:]])virtualbox($|[[:space:]]) ]]; then
        ...
        exec_log "sudo systemctl enable vboxservice" "Activation of vboxservice"
	...
	if [[ ${vm} =~ (^|[[:space:]])vmware($|[[:space:]]) ]]; then
        ...
        exec_log "sudo systemctl enable vmtoolsd" "Activation of 'vmtoolsd'"
		exec_log "sudo systemctl enable vmware-vmblock-fuse" "Activation of 'vmware-vmblock-fuse'"
