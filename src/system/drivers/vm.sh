...
function vm_drivers() {
	...
	if [[ ${vm} =~ (^|[[:space:]])vmware($|[[:space:]]) ]]; then
        ...
        exec_log "sudo systemctl enable vmtoolsd" "$(eval_gettext "activation of vmtoolsd")"
		exec_log "sudo systemctl enable vmware-vmblock-fuse" "$(eval_gettext "activation of vmware-vmblock-fuse")"
