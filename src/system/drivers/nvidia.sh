...
function nvidia_earlyloading () {
    if ! grep -q 'nvidia_drm' /etc/mkinitcpio.conf; then
        ...
        log "echo NVIDIA modules already present in mkinitcpio.conf"
    fi
}

# Optional installation of Intel GPU drivers for hybrid laptops
function nvidia_intel() {
    if ask_question "echo Do you have an Intel/Nvidia Laptop ?"; then
        local -r inlst="
            intel-media-driver
            intel-gmmlib
            onevpl-intel-gpu
...
function nvidia_drivers() {
    ...
    uninstall_lst "${unlst}" "echo Clean old NVIDIA driver dependencies"
    ...
    if ask_question "echo Do you want to install CUDA (${RED}say No if unsure${RESET}) ?"; then
        install_one "cuda"
    fi
    ...
    if ((device_type >= 8 && device_type <= 11)); then
        ...
        if ! lspci -d "10de:*:030x" -vm | grep -q 'Device:\s*TU'; then
            ...
            log "echo NVIDIA PowerD is not supported on Turing GPUs"
        fi
    else
        log "echo Not a laptop chassis; skipping NVIDIA PowerD"
    fi
}
